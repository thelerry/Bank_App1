...
class Person:
    ...
    def privet(self):
        a = name.get()
        messagebox.showinfo("dfgh", f"{a}")

    def poka(self):
        b = surname.get()
        messagebox.showinfo("asdf", f"{b}")

    def kakdela(self):
        c = age.get()
        messagebox.showinfo("sdef", f"{c}")

    def hello(self):
        d = password.get()
        messagebox.showinfo("sdep", f"{d}")
...
btn = Button(frame, text="", command=a.privet)
btn.grid(row=5, column=2)

frame = Frame(window, padx=10, pady=11)
frame.pack(expand=True)
surname = Entry(frame)
surname.grid(row=5, column=3)
btn = Button(frame, text="", command=b.poka)
...
btn = Button(frame, text="", command=c.kakdela)
...
btn = Button(frame, text="", command=d.hello)
